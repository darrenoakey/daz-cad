...
def export_assembly_to_format(assembly, export_format):
    ...
    # For assemblies, we use the save method which works for STL and STEP
    format_map = {
...
def export_shape_to_format(shape, export_format):
    ...
    # Get file extension and export type - only verified working formats
    format_map = {
        'stl': ('.stl', cq.exporters.ExportTypes.STL),
        'step': ('.step', cq.exporters.ExportTypes.STEP)
    }
...
class TestExportFunctions(unittest.TestCase):
    ...
    def test_export_assembly_to_format(self):
        ...
        # Test each supported format (using save method)
        for fmt in ['stl', 'step']:
